...
public class ZigZagConversion6 {
  ...
  static class Solution2 {
    ...
    public String convert(String s, int numRows) {
      ...
      int numGroups = finalGroupSize == 0 ? s.length() / groupSize : s.length() / groupSize + 1;

      for (int rowIndex = 0; rowIndex < numRows; rowIndex++) {
        for (int groupIndex = 0; groupIndex < numGroups; groupIndex++) {
          int columnCharacterIndex = rowIndex + groupIndex * groupSize;
          if (columnCharacterIndex < s.length()) {
            result.append(s.charAt(columnCharacterIndex));
          }
          int interColumnCharacterIndex = (groupSize - rowIndex) + groupIndex * groupSize;

          if (rowIndex != 0 && rowIndex != numRows - 1 && interColumnCharacterIndex < s.length()) {
